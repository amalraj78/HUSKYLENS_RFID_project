...
photo=PhotoImage(file="/home/pi/Downloads/principal.png",master=root)

label = Label(root,image= photo) #Background
label.pack()


register_button = tk.Button(root, text="Enregistrer Tag/Visage/Nom", command=register_user_window, height=2, width=20)
register_button.pack(pady=10)
register_button.place(x=920, y=420)

recognize_button = tk.Button(root, text="Authentification", command=recognize_user_window,height=2,width=20)
recognize_button.pack(pady=10)
recognize_button.place(x=920, y=475)

quit_button = tk.Button(root, text="Quitter", command=root.quit,height=2,width=20,bg='red')
quit_button.pack(pady=10)
quit_button.place(x=920, y=530)
